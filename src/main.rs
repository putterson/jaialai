...
struct Physics {
    world: World<f32>,
    handles: Vec<RigidBodyHandle<f32>>,
}

impl Physics {
    fn new() -> Physics {
        let (world, handles) = Physics::init();
    ...
    fn init() -> (World<f32>, Vec<RigidBodyHandle<f32>>) {
        /*
         * World
         */
        ...
        let x = 0 as f32 * 2.5 * rad - centerx;
        let y = 10.0 + 0 as f32 * 2.5 * rad + centery * 2.0;
        let z = 0 as f32 * 2.5 * rad - centerx;

        let mut rb = RigidBody::new_dynamic(Ball::new(rad), 1.0, 0.3, 0.6);

        rb.append_translation(&Translation3::new(x, y, z));

        let rbh = world.add_rigid_body(rb);
        rbs.push(rbh);
...
impl event::EventHandler for MainState {
    fn update(&mut self, _ctx: &mut Context) -> GameResult<()> {
        ...
        if let Some(first) = self.physics.handles.first() {
            let rb = first.borrow();
            self.pos_x = rb.position().translation.vector.x.abs() * 50.0;
            self.pos_y = -rb.position().translation.vector.z * 50.0;
        }
